...
import { Row, Card, Input, Select, Table, Button, DatePicker, Dropdown, Menu, Icon, Tooltip,Col, Modal } from 'antd';
...
export default class SaleAgent extends PureComponent {
  ...
  handleConfirmCancel=()=>{
    const { dispatch } = this.props;
    dispatch({
      type:'saleOrderList/confirmCancel',
    })
  }
  render() {
    const {
            ...
            <Dropdown
              overlay={ 
                <Menu>
                  ...
                  <Menu.Item>
                    <div onClick={this.handleCancel.bind(this,record.orderId)}>
                      <Icon type="delete" /> 取消
                    </div>
                  </Menu.Item>
                  {
      ...
      <PageHeaderLayout title="一件代发">
        <Card bordered={false}>
          <Row style={{ position: 'relative' }}>
            ...
            <Col span={4}>
              <Button type="primary" onClick={this.handleFastCombin} loading={buttonLoading}>一键合成</Button>
            </Col>
          </Row>
          <Row>
            <Col span={6}>
