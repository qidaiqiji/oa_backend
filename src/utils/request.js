...
export function getUrl(mode) {
  const urls = {
    ...
    // testerp: 'https://testerp.xiaomei360.com',
  };
  return urls[mode];
}
...
export default function request(url, options = {}, isConveyToDev = false) {
  ...
  // console.log(sessionStorage.getItem('token'), url);
  // if (sessionStorage.getItem('token') == null && url !== '/erp-site/login') {
  //   window.location.href = 'http://oa.xiaomei360.com/user/login';
  // }
  const newOptions = API_ENV === 'mock' ? { ...options } : { ...defaultOptions, ...options };
