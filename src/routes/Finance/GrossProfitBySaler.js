...
@connect(state => ({
    grossProfitBySaler: state.grossProfitBySaler,
}))
export default class grossProfitBySaler extends PureComponent {
    componentDidMount() {
        const { dispatch } = this.props;
        dispatch({
            type: 'grossProfitBySaler/getConfig',
        });
        dispatch({
            type: 'grossProfitBySaler/getList',
        });
    }
    componentWillUnmount() {
        const { dispatch } = this.props;
        dispatch({
            type: 'grossProfitBySaler/unmountReducer',
        });
    }
    handleChangeMenu=(e)=>{
        const { dispatch } = this.props;
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: '销售分组',
        dataIndex: 'sellerGroupId',
        key: 'sellerGroupId',
        align: 'center',
        render:(sellerGroupId)=>{
            return <span>{sellerTeamMap[sellerGroupId]}</span>
        }
      },
      {
        title: '销售数量',
            ...
            <Row gutter={16} style={{ marginTop:10 }}>
                <Col span={20}>
                    ...
                    <Select
                        ...
                        onChange={this.handleSearchByItems.bind(this,'sellerId')}
                    >
                        <Option value="">全部</Option>
                        {
                            Object.keys(sellerMap).map(item=>{
                                return <Option key={item}>{sellerMap[item]}</Option>
                            })
                        }
                    </Select>
                    <Select
                        ...
                        placeholder="选择销售分组"
                        onChange={this.handleSearchByItems.bind(this,'sellerGroupId')}
                    >
                        <Option value="">全部</Option>
                        {
                            Object.keys(sellerTeamMap).map(item=>{
                                return <Option key={item}>{sellerTeamMap[item]}</Option>
                            })
                        }
                    </Select>
                    <RangePicker
