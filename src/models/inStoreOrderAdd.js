...
import { reqGetConfig, reqGetGoods, reqSaveOrder } from '../services/inStoreOrderAdd';

export default {
  namespace: 'inStoreOrderAdd',
  ...
  effects: {
    *getConfig(_, { put, call }) {
      try {
        const res = yield call(reqGetConfig, { add: 1 });
        yield put({
          type: 'getConfigReducer',
          payload: {
            inStoreTypeMap: res.data.inStoreTypeMap,
            storageTypeMap: res.data.storageTypeMap,
          },
        });
      } catch (error) {
        // todo
    ...
    *changeInStoreType({ payload }, { put }) {
      yield put({
        type: 'changeInStoreTypeReducer',
        payload: {
          ...payload,
        },
      });
    ...
    *changeInStoreNum({ payload }, { put }) {
      yield put({
        type: 'changeInStoreNumReducer',
        payload: {
          ...payload,
        },
      });
    ...
    *showMoreGoods({ payload }, { put, call }) {
      try {
        ...
        for (let i = 0; i < goodsData.data.goods.length; i += 1) {
          goodsData.data.goods[i].inStoreNum = 0;
        }
        yield put({
    ...
    *changeRemarkValue({ payload }, { put }) {
      yield put({
        type: 'changeRemarkValueReducer',
        payload: {
          ...payload,
        },
      });
    ...
    *clickSaveBtn({ payload }, { put, call }) {
      try {
        ...
        yield put(routerRedux.push('/warehouse/in-store-order-list'));
      } catch (error) {
        // dodo
      }
    },
  ...
  reducers: {
    ...
    changeRemarkValueReducer(state, { payload }) {
      return {
        ...state,
        ...payload,
      };
    ...
    changeInStoreNumReducer(state, action) {
      const { payload } = action;
      ...
      if (isMoreGoods) {
        for (let i = 0; i < siftGoods.length; i += 1) {
          if (siftGoods[i].goodsId === payload.goodsId) {
            siftGoods[i].inStoreNum = payload.number;
            break;
          }
        }
      } else {
        for (let i = 0; i < goodsInfos.length; i += 1) {
          if (goodsInfos[i].goodsId === payload.goodsId) {
            goodsInfos[i].inStoreNum = payload.number;
            break;
          }
        }
      }
    ...
    changeInStoreTypeReducer(state, { payload }) {
      return {
        ...state,
        ...payload,
      };
